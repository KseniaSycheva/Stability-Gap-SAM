...
import numpy as np
import src.models.conv.layers as conv_layers
...
class ConvLayers(nn.Module):
    ...
    def __init__(
        ...
        if conv_type == "resNet":
            num_blocks = (
                [num_blocks] * (depth - 1) if type(num_blocks) == int else num_blocks
            )
            assert len(num_blocks) == (depth - 1)
            block = (
                conv_layers.Bottleneck
                if block_type == "bottleneck"
                else conv_layers.BasicBlock
            )
        ...
        for layer_id in range(1, depth + 1):
            ...
            if conv_type == "standard" or layer_id == 1:
                conv_layer = conv_layers.conv_layer(
                    ...
                    batch_norm=False
                    if output == "none" and layer_id == depth
                    else batch_norm,
        ...
        self.pooling = (
            nn.AdaptiveAvgPool2d((1, 1)) if global_pooling else modules.Identity()
        )
...
class DeconvLayers(nn.Module):
            ...
            for layer_id in range(1, self.depth + 1):
                ...
                if deconv_type == "standard":
                    ...
                    new_layer = conv_layers.deconv_res_layer(
                        ...
                        nl=nl,
                        smaller_kernel=smaller_kernel,
                        output="normal" if layer_id < self.depth else output,
                    )
    ...
    def image_size(self, in_units):
        ...
        if self.depth > 0:
            ...
            )  # -> size of image fed to last layer (seen from image)
