...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, in_planes, planes, stride=1, batch_norm=True, nl="relu", no_fnl=False
    ):
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.block_layer1 = nn.Sequential(
            nn.Conv2d(
                in_planes, planes, kernel_size=1, bias=False if batch_norm else True
            ),
            nn.BatchNorm2d(planes) if batch_norm else modules.Identity(),
            nn.ReLU() if nl == "relu" else nn.LeakyReLU(),
        )
    ...
    def __init__(
        ...
        if output == "sigmoid":
            ...
            raise NotImplementedError(
                "Ouptut '{}' not implemented for deconvolutional ResNet layer.".format(
                    output
                )
            )
