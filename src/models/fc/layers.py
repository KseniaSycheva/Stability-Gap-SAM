...
from src.models.utils import modules
...
class fc_layer(nn.Module):
    ...
    def __init__(
        ...
        self.linear = em.LinearExcitability(
            in_size,
            out_size,
        ...
        if isinstance(nl, nn.Module):
            ...
            self.nl = (
                nn.ReLU()
                if nl == "relu"
                else (nn.LeakyReLU() if nl == "leakyrelu" else modules.Identity())
            )
...
class fc_layer_fixed_gates(nn.Module):
    ...
    def forward(self, x, gate_input=None, return_pa=False):
        ...
        gate = (
            torch.matmul(gate_input, self.gate_mask)
...
class fc_multihead_layer(nn.Module):
    ...
    def __init__(
        ...
        if n_contexts > 0:
            self.gate_mask = torch.zeros(
                size=(n_contexts, classes), dtype=torch.float, device=device
            )
            classes_per_context = int(classes / n_contexts)
            for context_id in range(n_contexts):
                self.gate_mask[
                    context_id,
                    (context_id * classes_per_context) : (
                        (context_id + 1) * classes_per_context
                    ),
                ] = 1.0
        if isinstance(nl, nn.Module):
            self.nl = nl
