...
import numpy as np
from src.models.utils.modules import Identity
from src.models.fc.layers import fc_layer, fc_layer_fixed_gates
...
class MLP_gates(nn.Module):
    ...
    def __init__(
        ...
        nd_label = "{drop}{bias}{exc}{bn}{nl}{gate}".format(
            ...
            bn="b" if batch_norm else "",
            nl="l" if nl == "leakyrelu" else ("n" if nl == "none" else ""),
            gate="g{}m{}".format(gate_size, gating_prop)
            if (gate_size > 0 and gating_prop > 0.0)
            else "",
        )
        nd_label = "{}{}".format(
            "" if nd_label == "" else "-{}".format(nd_label),
        ...
        for lay_id in range(1, self.layers + 1):
            ...
            if (
                (not gate_size > 0.0)
                or (not gating_prop > 0.0)
                or (lay_id == self.layers and not final_gate)
            ):
                layer = fc_layer(
                    in_size,
                    out_size,
    ...
    def forward(
        self, x, gate_input=None, skip_first=0, skip_last=0, return_lists=False
    ):
        ...
        for lay_id in range(skip_first + 1, self.layers + 1 - skip_last):
            (x, pre_act) = getattr(self, "fcLayer{}".format(lay_id))(
                x, gate_input=gate_input, return_pa=True
            )
            if return_lists:
