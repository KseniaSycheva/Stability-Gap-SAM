...
from src.models.fc.layers import fc_layer
from src.models.fc.nets import MLP
from src.models.conv.nets import ConvLayers
from src.models.cl.memory_buffer import MemoryBuffer
from src.models.cl.continual_learner import ContinualLearner
from src.models.utils import loss_functions as lf, modules
...
class Classifier(ContinualLearner, MemoryBuffer):
    ...
    def __init__(
        ...
        self.fcE = MLP(
            input_size=self.conv_out_units,
    ...
    def train_a_batch(
        ...
        **kwargs,
        ...
        gradient_per_context = (
            True if ((self.mask_dict is not None) and (x_ is not None)) else False
        )
        ...
        if x_ is not None:
            ...
            PerContext = (
                (type(y_) == list) if (y_ is not None) else (type(scores_) == list)
            )
            if not PerContext:
                y_ = [y_]
                scores_ = [scores_]
                active_classes = (
                    [active_classes] if (active_classes is not None) else None
                )
            ...
            for replay_id in range(n_replays):
                ...
                if (y_ is not None) and (y_[replay_id] is not None):
                    if self.binaryCE:
                        binary_targets_ = lf.to_one_hot(
                            y_[replay_id].cpu(), y_hat.size(1)
                        ).to(y_[replay_id].device)
                        predL_r[replay_id] = (
                        ...
                        predL_r[replay_id] = F.cross_entropy(
                            y_hat, y_[replay_id], reduction="mean"
                        )
                ...
                if gradient_per_context:
                    weight = 1.0 if self.use_replay == "inequality" else (1.0 - rnt)
                    weighted_replay_loss_this_context = (
                        weight * loss_replay[replay_id] / n_replays
                    )
        ...
        if x is not None:
            ...
            if self.binaryCE:
                ...
                if self.binaryCE_distill and (scores is not None):
                    ...
                    binary_targets[:, : scores.size(1)] = torch.sigmoid(
                        scores / self.KD_temp
                    )
                predL = (
                    None
                ...
                predL = (
                    None
                    if y is None
                    else F.cross_entropy(input=y_hat, target=y, reduction="mean")
                )
            ...
            accuracy = (
                None if y is None else (y == y_hat.max(1)[1]).sum().item() / x.size(0)
            )
        else:
            accuracy = predL = None
        ...
        if self.precondition:
            if self.importance_weighting == "fisher" and not self.fisher_kfac:
                ...
                def scale_grad(label, layer):
                    ...
                    if linear.bias is not None:
                        linear.weight.grad = (
                            scaled_g[..., 0:-1].detach() / self.data_size
                        )
                    ...
                    if not hasattr(layer, "phantom"):
                        raise ValueError(
                            f"Layer {label} does not have phantom parameters"
                        )
        ...
        return {
            ...
            if (loss_replay is not None) and (x is not None)
            else 0,
            "pred": predL.item() if predL is not None else 0,
            "pred_r": sum(predL_r).item() / n_replays
            if (x_ is not None and predL_r[0] is not None)
            else 0,
            "distil_r": sum(distilL_r).item() / n_replays
            if (x_ is not None and distilL_r[0] is not None)
            else 0,
            "param_reg": weight_penalty_loss.item()
            if weight_penalty_loss is not None
            else 0,
