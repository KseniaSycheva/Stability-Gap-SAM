...
def loss_fn_kd(scores, target_scores, T=2.0, weights=None, norm=False):
    ...
    KD_loss_unnorm = weighted_average(
        KD_loss_unnorm, weights=weights, dim=0
    )  # -> average over batch
...
def loss_fn_kd_binary(scores, target_scores, T=2.0, weights=None, norm=False):
    ...
    # Calculate distillation loss (see e.g., Li and Hoiem, 2017)
    KD_loss_unnorm = -(
        targets_norm * torch.log(scores_norm)
        + (1 - targets_norm) * torch.log(1 - scores_norm)
    )
...
def log_Bernoulli(x, mean, average=False, dim=None):
    ...
    NOTES: [dim]=-1    summing / averaging over all but the first dimension
           [dim]=None  summing / averaging is done over all dimensions"""
    probs = torch.clamp(mean, min=1e-5, max=1.0 - 1e-5)
    ...
    if average:
        return (
            ...
            if dim is not None
            else torch.mean(log_bernoulli)
        )
    else:
        return (
            torch.sum(log_bernoulli, dim)
            if dim is not None
            else torch.sum(log_bernoulli)
        )
