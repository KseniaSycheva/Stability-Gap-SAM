...
class MemoryBuffer(nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(self):
        ...
        self.budget = (
            100  # -> this is the overall budget (there is not memory buffer per class)
        )
        ...
        self.compute_means = (
            True  # -> whenever new data is added, class-means must be recomputed
        )
    ...
    def initialize_buffer(self, config, return_c=False):
        ...
        self.buffer_y = torch.zeros(
            self.budget, dtype=torch.int64, device=self._device()
        )
        if return_c:
            self.buffer_c = torch.zeros(
                self.budget, dtype=torch.int64, device=self._device()
            )
        pass
    ...
    def sample_contexts(self, size):
        if len(self.contexts_so_far) == 0:
            ...
            return torch.tensor(
                np.random.choice(self.contexts_so_far, size, replace=True)
            )
    ...
    def classify_with_prototypes(self, x, context=None):
        ...
        if self.compute_means:
            ...
            for y in range(self.classes):
                if y in self.buffer_y:
                    ...
                    memory_set_means.append(
                        mu_y.squeeze()
                    )  # -> squeeze removes all dimensions of size 1
                else:
                    memory_set_means.append(
                        None
                    )  # to indicate that this class is not present in the memory buffer
        ...
        means = torch.stack(
            [means] * batch_size
        )  # (batch_size, n_possible_classes, feature_size)
        ...
        feature = feature.expand_as(
            means
        )  # (batch_size, feature_size, n_possible_classes)

        # For each sample in [x], find the (negative) distance of its extracted features to exemplar-mean of each class
        scores = (
            -(feature - means).pow(2).sum(dim=1).squeeze()
        )  # (batch_size, n_possible_classes)

        # For all classes not in the memory, return a score of [-inf]
        all_scores = (
            torch.ones(batch_size, self.classes, device=self._device()) * -np.inf
        )
