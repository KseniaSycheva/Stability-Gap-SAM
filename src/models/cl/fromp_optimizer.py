...
from src.models.fc import excitability_modules as em
...
def _check_param_device(param, old_param_device):
    if old_param_device is None:
        ...
        warn = (
            (param.get_device() != old_param_device)
            if param.is_cuda
            else (old_param_device != -1)
        )
        if warn:
            raise TypeError(
                "Parameters are on different devices, not currently supported."
            )
...
class opt_fromp(Optimizer):
    ...
    def cac_jacobian(self, output, lc):
        ...
        for i in range(num_fun):
            ...
            grad.append(g)
        result = torch.zeros(
            (grad[0].shape[0], grad[0].shape[1], num_fun),
            dtype=grad[0].dtype,
    ...
    def init_context(
        self, context_id, eps=1e-6, reset=True, classes_per_context=2, label_sets=None
    ):
        ...
        covariance = 1.0 / (fisher + prior_prec)  # -> size: [n_params]
        ...
        for i in range(context_id if self.per_context else 1):
            ...
            classes_in_context = (
                range(classes_per_context * i, classes_per_context * (i + 1))
                if self.per_context
                else range(classes_per_context * context_id)
            )
            mem_points_np = np.concatenate(
                [self.model.memory_sets[id] for id in classes_in_context], axis=0
            )
            memorable_points_t = torch.from_numpy(mem_points_np).to(
                self.model._device()
            )
            ...
            preds = torch.softmax(
                preds, dim=-1
            )  # -> size: [n_per_context]x[classes_per_context]
            ...
            for class_id in range(num_classes):
                ...
                grad = self.cac_grad(
                    loss, lc, retain_graph=retain_graph
                )  # -> size: [n_mem_points]x[n_params]
                with torch.no_grad():
                    kernel = (
                        torch.einsum("ij,j,pj->ip", grad, covariance, grad)
                        + torch.eye(grad.shape[0], dtype=grad.dtype, device=grad.device)
                        * eps
                    )
    ...
    def step(self, x, y, label_sets, context_id, classes_per_context):
        ...
        logits = (
            ...
            if (label_sets[context_id] is None)
        ...
        if context_id > 0:
            ...
            for t in range(context_id if self.per_context else 1):
                ...
                batch_size_per_context = (
                    int(np.ceil(x.shape[0] / context_id))
                    if self.per_context
                    else x.shape[0]
                )
                if self.per_context:
                    memory_samples_per_context = (
                        len(self.model.memory_sets[0]) * classes_per_context
                    )
                else:
                    memory_samples_per_context = (
                        len(self.model.memory_sets[0])
                        * classes_per_context
                        * context_id
                    )
                if batch_size_per_context < memory_samples_per_context:
                    indeces_in_this_batch = random.sample(
                        range(memory_samples_per_context), batch_size_per_context
                    )
                else:
                    indeces_in_this_batch = list(range(memory_samples_per_context))

                if self.per_context:
                    classes_in_context = range(
                        classes_per_context * t, classes_per_context * (t + 1)
                    )
                else:
                ...
                memorable_data_t = torch.from_numpy(
                    mem_points_np[indeces_in_this_batch]
                ...
                for fi in range(num_fun):
                    ...
                    delta_logits = (
                        logits_t - memorable_logits[t][indeces_in_this_batch, fi]
                    )

                    # K_{t-1}^{-1}
                    kernel_inv_t = kernel_inv[t][fi][:, indeces_in_this_batch][
                        indeces_in_this_batch
                    ]

                    # Calculate K_{t-1}^{-1} (m_t - m_{t-1})
                    with torch.no_grad():
                        kinvf_t = torch.squeeze(
                            torch.matmul(kernel_inv_t, delta_logits[:, None]), dim=-1
                        )
