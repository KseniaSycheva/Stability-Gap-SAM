...
from src.models.utils.ncl import additive_nearest_kf
...
class ContinualLearner(nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(self):
        ...
        ]  # -> lists the parameters to regularize with SI or diagonal Fisher
        ...
        self.singlehead = (
            False  # if Task-IL, does the model have a single-headed output layer?
        )
        self.neg_samples = (
            "all"  # if Class-IL, which output units should be set to 'active'?
        )
        ...
        self.KD_temp = 2.0  # temperature for distillation loss
        self.use_replay = (
            "normal"  # how to use the replayed data? (normal|inequality|both)
        )
        ...
        self.eps_agem = 0.0  # parameter that improves numerical stability of AGEM (if set slighly above 0)
        ...
        self.importance_weighting = (
            "fisher"  # -> Options for estimation of parameter importance:
        )
        ...
        self.fisher_batch = (
            1  # -> batch size for estimating FI-matrix (should be 1, for best results)
        )
        ...
        self.offline = (
            False  # -> use separate penalty term per context (as in original EWC paper)
        )
        self.gamma = 1.0  # -> decay-term for old contexts' contribution to cummulative FI (as in 'Online EWC')
    ...
    def apply_XdGmask(self, context):
        ...
        for i, excit_buffer in enumerate(self.excit_buffer_list):
            ...
            gating_mask[self.mask_dict[context][i]] = (
                0.0  # -> find context-specific mask
            )
            ...
            for n, p in gen_params():
                if p.requires_grad:
                    ...
                    self.register_buffer(
                        "{}_SI_prev_context".format(n), p.detach().clone()
                    )
        ...
        for gen_params in self.param_list:
            for n, p in gen_params():
                if p.requires_grad:
                    ...
                    self.register_buffer(
                        "{}_EWC_prev_context".format(n), p.detach().clone() * 0
                    )
    ...
    def estimate_fisher(self, dataset, allowed_classes=None):
        ...
        data_loader = get_data_loader(
            dataset,
            batch_size=1 if self.fisher_batch is None else self.fisher_batch,
        ...
        for index, (x, y) in enumerate(data_loader):
            ...
            if self.fisher_labels == "all":
                ...
                with torch.no_grad():
                    label_weights = F.softmax(
                        output, dim=1
                    )  # --> get weights, which shouldn't have gradient tracked
                for label_index in range(output.shape[1]):
                    label = torch.LongTensor([label_index]).to(self._device())
                    negloglikelihood = F.cross_entropy(
                        output, label
                    )  # --> get neg log-likelihoods for this class
                    # Calculate gradient of negative loglikelihood
                    self.zero_grad()
                    negloglikelihood.backward(
                        retain_graph=True
                        if (label_index + 1) < output.shape[1]
                        else False
                    )
                    ...
                    for gen_params in self.param_list:
                        for n, p in gen_params():
                            if p.requires_grad:
                                n = n.replace(".", "__")
                                if p.grad is not None:
                                    est_fisher_info[n] += label_weights[0][
                                        label_index
                                    ] * (p.grad.detach() ** 2)
                ...
                if self.fisher_labels == "true":
                    ...
                    label = (
                        torch.LongTensor([y]) if type(y) == int else y
                    )  # -> shape: [self.fisher_batch]
                    ...
                    weights_array = np.array(
                        label_weights[0].cpu()
                    )  # --> change to np-array, avoiding rounding errors
                    label = np.random.choice(
                        len(weights_array), 1, p=weights_array / weights_array.sum()
                    )
                    label = torch.LongTensor(label).to(
                        self._device()
                    )  # --> change label to tensor on correct device
        ...
        for gen_params in self.param_list:
            for n, p in gen_params():
                if p.requires_grad:
                    ...
                    self.register_buffer(
                        "{}_EWC_prev_context{}".format(
                            n, self.context_count + 1 if self.offline else ""
                        ),
                        p.detach().clone(),
                    )
                    # -precision (approximated by diagonal Fisher Information matrix)
                    if (not self.offline) and hasattr(
    ...
    def ewc_loss(self):
        ...
        try:
            ...
            num_penalty_terms = (
                self.context_count if (self.offline and self.context_count > 0) else 1
            )
                    ...
                    for n, p in gen_params():
                        if p.requires_grad:
                            ...
                            mean = getattr(
                                self,
                                "{}_EWC_prev_context{}".format(
                                    n, context if self.offline else ""
                                ),
                            )
                            fisher = getattr(
                                self,
    ...
    def estimate_kfac_fisher(self, dataset, allowed_classes=None):
        ...
        def update_fisher_info_layer(
            est_fisher_info, intermediate, label, layer, n_samples, weight=1
        ):
            if not isinstance(layer, fc.layers.fc_layer):
                raise NotImplemented
            ...
            if self.fisher_labels == "all":
                ...
                for label_index in range(output.shape[1]):
                    ...
                    update_fisher_info(
                        ...
                        intermediate,
                        n_samples,
                        weight=label_weights[0][label_index],
                    )
            else:
                # -only use one particular label for each datapoint
                if self.fisher_labels == "true":
        ...
        for label in est_fisher_info:
            ...
            Ao = self.gamma * self.KFAC_FISHER_INFO[label]["A"].to(
                self._device()
            )  # old kronecker factor
            Go = self.KFAC_FISHER_INFO[label]["G"].to(
                self._device()
            )  # old kronecker factor

            As, Gs = additive_nearest_kf(
                {"A": Ao, "G": Go}, {"A": An, "G": Gn}
            )  # sum of kronecker factors
    ...
    def estimate_owm_fisher(self, dataset, **kwargs):
        ...
        def update_fisher_info(est_fisher_info, intermediate, n_samples):
            for i in range(1, fcE.layers + 1):
                label = f"fcLayer{i}"
                update_fisher_info_layer(
                    est_fisher_info, intermediate, label, n_samples
                )
            update_fisher_info_layer(
                est_fisher_info, intermediate, "classifier", n_samples
            )
