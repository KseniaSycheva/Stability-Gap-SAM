...
from src.models.cl.fromp_optimizer import softmax_hessian
...
class MemoryBuffer(nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(self):
        ...
        self.prototypes = (
            False  # -> perform classification by using prototypes as class templates
        )
        self.add_buffer = (
            False  # -> add the memory buffer to the training set of the current task
        )
    ...
    def construct_memory_set(self, dataset, n, label_set):
        ...
        if self.sample_selection == "fromp":
            ...
            for k in range(min(n, n_max)):
                if k > 0:
                    selected_samples_sum = torch.sum(
                        selected_features[:k], dim=0
                    ).unsqueeze(0)
                ...
                if index_selected in list_of_selected:
                    raise ValueError(
                        "Samples in the memory buffer should not be repeated!!!!"
                    )
    ...
    def classify_with_prototypes(self, x, allowed_classes=None):
        ...
        memory_set_means = (
            ...
            else [self.memory_set_means[i] for i in allowed_classes]
        )
